...
The following documents are structured progressively. The first covers foundational ECMP mechanics, the second builds on that with congestion-aware adaptive routing, and the third applies both to the specific constraints of RDMA transport in AI training fabrics.
...
- [Load Balancing for RoCEv2](03_README_ROCE_LB.md): Why RDMA's single-path Reliable Connection breaks standard load balancing at AI scale — the RC ordering constraint, QP scaling, adaptive routing for RoCE, the MRC multipath protocol, multi-plane topology co-design, SRv6 source routing, and cell-based switching.
